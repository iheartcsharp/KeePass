...
namespace KeePass.Util
{
    ...
    public static class WinUtil
    {
        ...
        public static string CompactPath(string strPath, int cchMax)
        {
            ...
            if (cchMax < 0)
            {
                throw new ArgumentOutOfRangeException("cchMax");
            }

            if (strPath.Length <= cchMax)
            {
                return strPath;
            }
        ...
        public static bool FlushStorageBuffers(char chDriveLetter, bool bOnlyIfRemovable)
        {
            ...
            try
            {
                ...
                if (strDir.Length > 0)
                {
                    WinUtil.SetWorkingDirectory(strDir);
                }
        ...
        public static bool FlushStorageBuffers(string strFileOnStorage, bool bOnlyIfRemovable)
        {
            ...
            if (strFileOnStorage.Length < 3)
            {
                return false;
            }

            if (strFileOnStorage[1] != ':')
            {
                return false;
            }

            if (strFileOnStorage[2] != '\\')
            {
                return false;
            }
        ...
        private static string FreeDriveIfCurrent(char chDriveLetter)
        {
            try
            {
                ...
                if ((strCur == null) || (strCur.Length < 3))
                {
                    return string.Empty;
                }
        ...
        public static bool IsInternetExplorer7Window(string strWindowTitle)
        {
            ...
            {
                return false; // No assert or throw
            }

            if (strWindowTitle.Length == 0)
            {
                return false; // No assert or throw
            }
            ...
            {
                if (strWindowTitle.IndexOf(str) >= 0)
                {
                    return true;
                }
            }
        ...
        public static ulong GetMaxNetFrameworkVersion()
        {
            ...
            if (m_uFrameworkVersion == 0)
            {
                ...
                if (v.Major > 0)
                {
                    m_uFrameworkVersion |= (uint)v.Major;
                }

                m_uFrameworkVersion <<= 16;
                if (v.Minor > 0)
                {
                    m_uFrameworkVersion |= (uint)v.Minor;
                }

                m_uFrameworkVersion <<= 16;
                if (v.Build > 0)
                {
                    m_uFrameworkVersion |= (uint)v.Build;
                }

                m_uFrameworkVersion <<= 16;
                if (v.Revision > 0)
                {
                    m_uFrameworkVersion |= (uint)v.Revision;
                }
            }
        ...
        private static ulong GetMaxNetVersionPriv()
        {
            ...
            foreach (string strInNdp in vInNdp)
            {
                ...
                if (!strInNdp.StartsWith("v", StrUtil.CaseIgnoreCmp))
                {
                    continue;
                }
        ...
        private static void UpdateNetVersionFromRegKey(RegistryKey k, ref ulong uMaxVer)
        {
            ...
            try
            {
                ...
                if ((strInstall.Length > 0) && (strInstall != "1"))
                {
                    return;
                }
